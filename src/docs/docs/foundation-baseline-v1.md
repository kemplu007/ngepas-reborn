...
15. F1 Product Decision-Quality Cues sudah dipromosikan sebagai penguatan hirarki kurasi public tanpa kontrak data baru. Iterasi berikutnya harus tetap satu slice, memakai data existing, dan tidak menambah klaim editorial otomatis.
16. Migrasikan halaman admin lain yang tersisa ke `Button`, `IconButton`, `Badge`, `Input`, `SelectField`, dan `Dialog`.
17. Hapus compatibility wrapper hanya setelah tidak ada runtime import ke `common/Button` dan seluruh build/review lulus.
