...
`Zero-Cost Media Runway` dipromosikan melalui PR #17 dan merge commit `2fd16d1` sebagai guardrail YAGNI di atas A3. Operasi admin tetap memakai URL `image` dan `gallery` existing; hosting gratis manual pihak ketiga hanya dapat dipakai sebagai cadangan di luar integrasi aplikasi. Ngepas tidak menambah native upload, WebP, provider storage, akun aplikasi, billing, secret, endpoint, dependency, schema, migration, scheduler, API runtime, atau perubahan JWT. A4 native upload + WebP dibekukan sampai kebutuhan operasional nyata dibuktikan melalui sekitar 20 produk lengkap, ritme input empat minggu, serta hambatan URL manual, kuota gratis, atau insiden gambar yang dapat ditinjau. Sesudahnya, setiap biaya dan provider tetap memerlukan approval founder terpisah.

`Admin Publishing Runbook` F0 dipromosikan melalui PR #19 dan merge commit `088d1dd` sebagai lapisan operasional di atas A1 Product Completeness Cues, A2 Gallery URL Assistant, A3 Media Upload Contract, dan guardrail Zero-Cost Media Runway. Runbook memberi SOP mobile-first bagi pengelola konten nonteknis untuk menyiapkan materi yang berhak dipakai, mengisi field kurasi yang jujur, memeriksa `4/4 siap`, menambahkan URL HTTPS `image`/`gallery`, menyimpan Draft, meninjau, lalu mempublikasikan dengan status existing. Ia juga mengunci checklist kualitas dan scorecard manual mingguan tanpa analytics atau scheduler. F0 tidak menambah state, handler, payload, validator, route, Context, service, API, backend, database, schema, persistence, upload, WebP, storage, provider, billing, secret, scheduler, auth JWT, atau data produksi. A4 tetap dibekukan hingga milestone produktif dan approval biaya terpisah terpenuhi.

`Product Decision-Quality Cues` F1 dipromosikan melalui PR #21 dan merge commit `c386a5b` sebagai layer visual-only pada Product Detail publik. `whyWeRecommend`, `bestFor`, dan `considerations` yang sudah berasal dari form admin disatukan ke Panduan keputusan sebelum CTA marketplace, sehingga pembaca melihat alasan pemilihan, kecocokan, dan pertimbangan tanpa informasi ganda. Render bersifat kondisional per subbagian dan pada container utamanya; produk yang belum memiliki field kurasi tidak mendapat card kosong, skor, label kualitas, maupun konten rekaan. CTA marketplace, sticky behavior mobile, gallery state, `useProducts`, `useParams`, route `/product/:slug`, Context, service, API, backend, database, schema, persistence, auth JWT, dan data produksi tetap tidak berubah. F1 memakai `Card`, `Badge`, Lucide, dan token visual existing; tidak menambah upload, WebP, storage, provider, billing, secret, scheduler, endpoint media, atau dependency. Validasi `git diff --check` dan `npx vite build` lulus, serta Product Detail diperiksa pada desktop dan mobile 375×812 menggunakan katalog produksi read-only tanpa overflow horizontal. Console preview memiliki warning key `/category` duplikat pada navigasi existing, di luar scope F1.

`Product Detail Curated-Data Polish` KEM-13 dipromosikan melalui PR #25 dan merge commit `44e31d6` sebagai polesan visual-only lanjutan pada Panduan keputusan F1. Slice ini memperjelas scan path serta pemisahan alasan pemilihan, kecocokan, dan pertimbangan menggunakan field manusia `whyWeRecommend`, `bestFor`, dan `considerations` yang sudah ada; fallback tetap jujur karena subbagian dan panel tidak dirender ketika data tidak tersedia. Lookup `useProducts`, route `/product/:slug`, gallery, price/meta, sticky CTA mobile, CTA desktop static, link affiliate, fallback CTA, detail, produk terkait, Context, service, API, backend, database, schema, persistence, auth JWT, dan data produksi tidak berubah. KEM-13 tidak menambah skor, review sintetis, label “terbaik”, upload, WebP, storage, provider, billing, secret, scheduler, endpoint media, atau dependency. Validasi `git diff --check` dan `npx vite build` lulus; route `/product/rak-bumbu-dapur` diperiksa read-only terhadap API produksi pada mobile 375×812 dan desktop 1280×720 tanpa horizontal overflow. Warning duplicate key `/category` pada navigasi existing tetap di luar scope.

`Product Detail by Slug` KEM-14 dipromosikan melalui PR #27 dan merge commit `2c57ac2` sebagai contract slice read-only untuk route detail publik. `GET /api/products/:slug` mengikuti jalur Route → Validator → Sanitizer → Controller → Product Model → SQLite; query parameterized berada hanya di model, sementara controller tetap menggunakan response helper dan parser produk existing. `productService` dan `ProductContext` menyediakan action lookup per-slug, sehingga `ProductDetail` meminta satu resource melalui state layer tanpa fetch langsung maupun menjadikan katalog penuh sebagai sumber detail utama. Response 200 sukses, 400 slug invalid, dan 404 produk tidak ditemukan sudah dibuktikan dengan harness controller read-only. KEM-14 tidak mengubah route `/product/:slug`, CTA marketplace, gallery, harga, Panduan keputusan KEM-13, related products, auth JWT, write flow, schema, migration, persistence, data produksi, upload, WebP, storage, billing, secret, scheduler, atau kebijakan visibility publik existing; filter Published/Draft tetap merupakan keputusan terpisah. Validasi `git diff --check`, `node --check` untuk file backend yang berubah, harness 200/400/404, dan `npx vite build` lulus.
...
`Draft Visibility Fix` KEM-30 dipromosikan melalui PR #33 dan merge commit `7b959a6`. Pada jalur publik, `GET /api/products/:slug` sekarang hanya membaca produk `published`, sehingga slug Draft menerima 404 yang sama seperti slug yang tidak ada. Halaman `/discover/:slug` juga memfilter Draft sebelum lookup detail, sedangkan produk terkait pada `/product/:slug` mengecualikan Draft. KEM-30 mempertahankan jalur admin list/context dan edit berbasis ID; tidak ada perubahan pada JWT, middleware auth, schema/migrasi SQLite, write flow, media, storage, WebP, billing, dependency, atau data produksi. Validasi mencakup `git diff --check`, syntax check backend, harness statis 3 assertion tanpa database, Vite build, deployment Vercel/Railway sukses, serta pemeriksaan read-only homepage, katalog publik, dan lookup slug Published. Raw `GET /api/products` belum dipisahkan menjadi kontrak admin/public; non-disclosure Draft pada raw list tetap memerlukan slice terpisah bila menjadi kebutuhan produk.
...
15. F1 Product Decision-Quality Cues sudah dipromosikan sebagai penguatan hirarki kurasi public tanpa kontrak data baru.
16. KEM-13 Product Detail Curated-Data Polish sudah dipromosikan sebagai polesan scan path visual-only; iterasi berikutnya tetap satu slice, memakai data existing, dan tidak menambah klaim editorial otomatis.
17. Migrasikan halaman admin lain yang tersisa ke `Button`, `IconButton`, `Badge`, `Input`, `SelectField`, dan `Dialog`.
18. Hapus compatibility wrapper hanya setelah tidak ada runtime import ke `common/Button` dan seluruh build/review lulus.
