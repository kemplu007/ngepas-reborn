/*==================================================
 NGEPAS REBORN
 File    : DiscoverPrototype.jsx
 Module  : Public Prototype Page
==================================================*/
...
import PrototypeHeader from "../../components/public/PrototypeHeader";
...
function PrototypeHero({ onSearch }) {
  return (
  ...
  return (
    <article className="group min-w-[230px] overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg sm:min-w-0">
      ...
      <div className="space-y-2 p-4">
        ...
        <Link to={`/prototype/discover/${product.slug}`} className="mt-2 flex items-center justify-center gap-1 rounded-xl border border-emerald-200 px-3 py-2 text-xs font-bold text-emerald-800 transition hover:bg-emerald-50">Lihat Detail <ArrowRight size={14} /></Link>
      </div>
    </article>
  );
}
...
function PrototypeDetail({ product, favorite, onFavorite }) {
  return (
    <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-10">
      <Link to="/prototype/discover" className="mb-6 inline-flex items-center gap-2 text-sm font-semibold text-slate-600 hover:text-emerald-700"><ArrowLeft size={17} /> Kembali ke Discover</Link>
...
function DiscoverPrototype() {
  ...
  const submitSearch = () => {
    ...
    document.getElementById("hasil-prototype")?.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  ...
  if (slug && selectedProduct) {
    return <><PrototypeHeader query={query} onQueryChange={setQuery} onSubmit={submitSearch} onFilter={openFilters} /><PrototypeDetail product={selectedProduct} favorite={favorites.includes(selectedProduct.slug)} onFavorite={toggleFavorite} /></>;
  }

  return (
    <div className="min-h-screen bg-white text-slate-900">
      <PrototypeHeader query={query} onQueryChange={setQuery} onSubmit={submitSearch} onFilter={openFilters} />
      <PrototypeHero onSearch={() => { document.getElementById("hasil-prototype")?.scrollIntoView({ behavior: "smooth" }); }} />
      <CategoryStrip categories={categories} selectedCategory={selectedCategory} onSelect={setSelectedCategory} />
      <section id="hasil-prototype" className="mx-auto max-w-7xl scroll-mt-24 px-4 py-6 sm:px-6 sm:py-10">
      ...
      <div className="fixed bottom-4 left-1/2 z-30 flex -translate-x-1/2 items-center gap-1 rounded-full border border-slate-200 bg-white/95 p-1 shadow-xl backdrop-blur lg:hidden"><Link to="/prototype/discover" className="rounded-full bg-emerald-50 px-3 py-2 text-[10px] font-bold text-emerald-800">Home</Link><button type="button" onClick={() => document.getElementById("hasil-prototype")?.scrollIntoView({ behavior: "smooth" })} className="rounded-full px-3 py-2 text-[10px] font-bold text-slate-600">Cari</button><Link to="/category" className="rounded-full px-3 py-2 text-[10px] font-bold text-slate-600">Kategori</Link><Link to="/prototype/discover#compare" className="rounded-full px-3 py-2 text-[10px] font-bold text-slate-600">Compare</Link></div>
    </div>
  );
}
...
export default DiscoverPrototype;
